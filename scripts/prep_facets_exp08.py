...
data = data[data[list(race_to_col.values())].fillna(0).any(axis=1)]
...
data['target_race'] = np.where(
    data[race_to_col['pacific_islander']] == 1, 5,
    np.where(
        data[race_to_col['native_american']] == 1, 4,
        np.where(
            data[race_to_col['asian']] == 1, 3,
            np.where(
                data[race_to_col['latinx']] == 1, 2,
                np.where(
                    data[race_to_col['middle_eastern']] == 1, 8,
                    np.where(
                        data[race_to_col['white']] == 1, 6,
                        np.where(
                            data[race_to_col['black']] == 1, 1, 7)))))))
...
facets = data[[comment_col, labeler_col, "target_race", "annotator_race", "item_id"] + items]
