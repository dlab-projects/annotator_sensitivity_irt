...
Experiment 13:
    ...
    - Recode responses
    - Use chosen targets, where the target is only a man or only a woman
    - Annotators are non-centered
...
data = data[data[keys.target_gender_cols].sum(axis=1) == 1]
...
for group in groups:
    ...
    with open(spec_path, 'w') as spec:
        spec.write(
            f"Title = Experiment 13\n"
            "Facets = 3\n"
            # Facet 1: Comments
            # Facet 2: Judges
            # Facet 3: Items (Partial Credit)
            f"Model = ?, ?, #, R\n"
            # Facet 2, the judges, is noncentered
            "Noncenter = 2\n"
